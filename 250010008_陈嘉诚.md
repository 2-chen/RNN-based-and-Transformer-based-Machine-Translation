...
- **训练集**：`train_100k.jsonl`（100,000个中英文句子对）
...
#### 2.1.2 分词
- **中文分词**：使用Jieba分词工具
- **英文分词**：使用NLTK的word_tokenize，并转换为小写
...
**未完成或需要改进的工作**：
1. ~~**RNN解码策略对比实验**：Beam Search已实现，但具体实验结果数据需要补充（3.3.3节显示"见实验记录"）~~ ✅ **已完成**：已补充完整的实验结果数据（见3.3.3节）
2. **Transformer的Beam Search解码实现**：已实现Beam Search解码功能，支持通过`--decode_method beam`参数使用
