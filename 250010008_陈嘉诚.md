...
│   ├── train_10k.jsonl     # Demo训练集（10k样本，用于快速测试）
...
│   └── t5_config.yaml       # T5模型配置
├── checkpoints/             # 模型检查点
│   ├── demo_rnn/            # RNN Demo模型（小规模，用于快速测试）
│   ├── demo_transformer/    # Transformer Demo模型（小规模，用于快速测试）
...
│   ├── t5_best/             # T5模型（10 epochs）
│   ├── t5_10epochs/         # T5模型（10 epochs训练结果）
│   └── bpe_tokenizer.json   # BPE分词器模型
├── models/                  # T5预训练模型（从Hugging Face下载）
├── visualizations/          # 可视化结果
│   ├── model_comparison.png # 模型性能对比图
│   ├── rnn_attention_comparison.png # RNN注意力机制对比
│   ├── rnn_training_policy_comparison.png # RNN训练策略对比
│   ├── transformer_norm_comparison.png # Transformer归一化方法对比
│   ├── attention_comparison_detailed.png # 注意力机制详细对比
│   ├── generalization_gap.png # 泛化能力对比
│   └── translation_examples.md # 翻译样例
├── experiment_results/      # 实验结果
│   ├── all_experiments_summary.json # 所有实验汇总
│   ├── all_results_collected.json # 收集的所有结果
│   ├── comparison_tables.md # 对比表格
│   ├── rnn_attention_comparison.json # RNN注意力机制对比结果
│   ├── rnn_training_policy_comparison.json # RNN训练策略对比结果
│   ├── transformer_hyperparameter_comparison.json # Transformer超参数对比
│   ├── transformer_norm_comparison.json # Transformer归一化方法对比
│   └── transformer_pos_encoding_comparison.json # Transformer位置编码对比
├── inference.py             # 单次推理脚本
├── train_demo.py            # Demo模型训练脚本（生成小规模模型用于测试）
├── evaluate_rnn_decoding.py  # RNN解码策略对比实验脚本
├── generate_visualizations.py # 可视化生成脚本
├── collect_results.py        # 实验结果收集脚本
├── run_experiments.py        # 批量实验运行脚本
├── test_inference.sh         # 一键测试脚本（Demo模型）
├── readme.md                 # 项目说明文档
├── requirements.txt          # Python依赖包
├── 如何获取模型文件.md        # 模型文件获取指南
├── 实验指南.md                # 实验操作指南
├── 实验训练指令.md            # 训练指令说明
└── 项目检查清单.md            # 项目检查清单
```
...
4. Papineni K, Roukos S, Ward T, et al. Bleu: a method for automatic evaluation of machine translation[C]//Proceedings of the 40th annual meeting of the Association for Computational Linguistics. 2002: 311-318.
